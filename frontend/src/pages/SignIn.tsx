...
export const SignIn: React.FC = () => {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      // User exists with email provider, proceed with signin
      const success = await signin(formData.email);
      if (success) {
        setStoredEmail(formData.email);
        // Navigate to OTP verification page for signin flow
        navigate('/otp-verification', { 
          state: { 
            email: formData.email,
            isSignup: false,
            message: 'Please enter the OTP sent to your email to sign in.'
          }
        });
      }
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    // For signin page, just send OTP and redirect
    await handleSendOTP();
  };
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      console.log('Got Google ID token, calling backend...');
      ...
      if (error.message.includes('not configured')) {
        toast.error('Google authentication is not properly set up. Please contact support.');
